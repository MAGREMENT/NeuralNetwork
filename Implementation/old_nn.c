...
inline old_nn* alloc_network(const int count, const int numbers[]){
    old_nn* result = malloc(sizeof(old_nn));
    result->count = count - 1;
    result->layers = malloc(result->count * sizeof(old_layer));
...
static void check_output_layer_delta_optimization(old_nn* network) {
...
inline void set_activation_type(old_nn* network, int type, int outputType) {
...
inline void set_cost_type(old_nn* network, int type) {
...
inline int get_cost_type(old_nn* network) {
...
inline void old_initialize(old_nn* network) {
...
inline void free_layers(old_layer* layers, int count) {
...
inline void free_network(old_nn* network){
...
inline void set_layer(old_layer layer, const double* weights, const double* biases){
...
inline backpropagation_data* alloc_back_data(const old_nn* network) {
...
inline void old_forward(old_layer layer, input_data input, input_data* result) {
...
inline void continue_advance(const old_layer layer, const backpropagation_data* data, const int inputIndex){
    for(int o = 0; o < layer.out_count; o++){
        double n = layer.biases[o];
...
inline void first_advance(const old_layer layer, const backpropagation_data* data, const input_data* input){
    for(int o = 0; o < layer.out_count; o++){
        double n = layer.biases[o];
...
inline void traverse(const old_nn* network, input_data* data, backpropagation_data* result){
...
inline backpropagation_data* alloc_traverse(const old_nn* network, input_data* data) {
    backpropagation_data* result = alloc_back_data(network);
    traverse(network, data, result);
    return result;
}

inline double cost(old_nn* network, input_data* data, input_data* expected) {
...
inline double avg_cost(old_nn* network, test_data* data) {
...
static void add_gradients_from_nv(const backpropagation_data* data,
        const old_nn* network, const layer_data* gradients, input_data input) {

    const int lastIndex = network->count - 1;

    for(int l = lastIndex; l >= 0; l--) {
        const old_layer current = network->layers[l];
...
inline void add_gradients(const old_nn* network, const layer_data* gradients, input_data input,
        const input_data expected) {
...
static void average_gradients(old_nn* network, layer_data* gradients, int dataCount) {
    for (int l = 0; l < network->count; l++) {
...
inline void learn(old_nn* network, test_data* data, range range, const double learningRate,
        void* optimizerState){
...
inline learning_state* alloc_state(old_nn* network) {
...
inline void free_state(old_nn* network, learning_state* state) {
...
inline test_result test_network(old_nn* network, test_data *test) {
    double valid = 0;
    for(int i = 0; i < test->count; i++) {
...
inline gradient_diagnostic* alloc_run_gradient_diagnostic(old_nn* network, test_data* data, int vanishingBound,
        int explodingBound) {
...
inline void print_diagnostic(old_nn* network, gradient_diagnostic* diag) {
    for (int i = 0; i < diag->scales->count; i++) {
